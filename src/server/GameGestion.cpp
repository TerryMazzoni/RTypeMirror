...
void GameGestion::updateGame(std::vector<std::pair<int, GenericCommunication>> communications)
{
    for (auto &entity : _entities) {
        ...
        if (entity.type != "__tile__") {
            for (auto &entity_colision: _entities) {
                if (entity_colision.type == "__tile__") {
                    // HANDLE COLLISION
                    if (std::any_cast<int>(entity.instance["x"]) == std::any_cast<int>(entity_colision.instance["x"])
                    && std::any_cast<int>(entity.instance["y"]) == std::any_cast<int>(entity_colision.instance["y"])) {
...
std::vector<std::pair<int, GenericCommunication>> GameGestion::getCommunications() const
{
